...
//var modisA1 = ee.ImageCollection("MODIS/061/MOD09A1") // this is 8 day and I can't get time from it!
...
/** Extract Sun Elevation/Sun Azimuth and Create as Property **/
...
function setSunElevModisViirs(imageCollection) {
  ...
  imageCollection = imageCollection.map(function(image) {
    return image.set("SUN_AZIMUTH", image.select("SolarAzimuth").reduceRegion({
      reducer: ee.Reducer.mean(),
      geometry: fluxFootprint,
      scale: image.get("pixelResolution")
    }).get("SolarAzimuth"));
  ...
  return imageCollection.map(function(image) {
    ...
    image = image.set("SUN_AZIMUTH", ee.Number(90).subtract(ee.Number(image.get("SUN_AZIMUTH")).divide(ee.Number(100))));
    return image;
  });
}

/** Extract AVHRR Time of Day **/
function setAvhrrTimeofday(imageCollection) {
  imageCollection = imageCollection.map(function(image) {
  ...
  return imageCollection.map(function(image) {
    return image.set("TIMEOFDAY", ee.Number(image.get("TIMEOFDAY")));
  });
}
...
function toFeature(image) {
  ...
  var platformID = ee.Dictionary({"Platform ID": ee.String(image.get("platformID"))});
  var time = ee.Dictionary({"Time": image.get("system:time_start")});
  var solarElevation = ee.Dictionary({"Solar Elevation": image.get("SUN_ELEVATION")});
  var solarAzimuth = ee.Dictionary({"Solar Azimuth": image.get("SUN_AZIMUTH")});
...
var imagerySeparate = ee.List([
  ...
  //  .map(setSunElevAster),
  //  
  //setSunElevModisViirs(modisGA)
  //  .map(setPixResProp(500))
  //  .map(maskModis),
  //  
  //setAvhrrTimeofday(setSunElevAvhrr(avhrr))
  // .map(setPixResProp(1000))
  // .map(maskAvhrr),
  //
  //setSunElevModisViirs(viirs)
...
// Test Print
print(ee.ImageCollection(imagerySeparate.get(0)).filterBounds(fluxFootprint).first())
...
Export.table.toDrive({
  ...
  fileFormat: "CSV",
  selectors: bands.concat(["Platform ID", "Solar Elevation", "Solar Azimuth", "Time"])
});